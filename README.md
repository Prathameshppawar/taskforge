...
| **Roles & teams** | Administrator-defined roles over a fixed permission catalogue, ranked so nobody can grant authority they do not hold; teams scope delegated people-management |
...
npm run verify    # 69 domain assertions — no DB, no network, <1s
npm run smoke     # signs in for real, walks every route, greps the server log
npm run e2e       # 15 browser tests, phone to desktop
